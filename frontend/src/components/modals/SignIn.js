...
import {ADMIN_ROUTE, SIGNUP_ROUTE} from '../../utils/consts'
...
const SignIn = observer(() => {
    ...
    const navigate = useNavigate()
        return (
            ...
            {user.isAuth ?
                <>
                <button type="button" class="btn btn-dark border border-2 ms-auto" onClick={() => navigate(ADMIN_ROUTE)}>
                    Админ панель
                </button>
                <button type="button" class="btn btn-dark border border-2 ms-4" onClick={() => user.setIsAuth(false)}>
                    Выйти
